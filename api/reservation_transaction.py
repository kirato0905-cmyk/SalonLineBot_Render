...
Calendar/Sheets inconsistencies.
...
5. save reservation to repository / Google Sheets
        ...
        if not saved:
            ...
            try:
                ...
                self._notify_critical(
                    title="予約不整合：Calendar作成済み / Sheets保存失敗 / rollback例外",
            ...
            if not rollback_success:
                self._notify_critical(
                    title="予約不整合：Calendar作成済み / Sheets保存失敗 / rollback失敗",
                    message="Google Calendarには予定が残っている可能性があります。Sheetsには保存されていません。手動確認してください。",
                    reservation_data=data,
                    error=save_error,
                )
                return {
            ...
            self._notify_warning(
                title="Calendar作成後にSheets保存失敗。rollback済み",
